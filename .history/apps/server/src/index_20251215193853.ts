...
io.on('connection', socket => {
    ...
    const auctionState = auctionStateByRoom[roomId]
    ...
    if (bidAmount > (me.purse || 0)) {
        socket.emit('errorMessage', 'Not enough purse')
        return
    }

    auctionState.currentBid = bidAmount
    auctionState.highestBidder = me.teamName || me.username
    ...
    auctionState.timer = setTimeout(() => finalizeBid(roomId), 15000)
    })
    ...
    const winner = room.players.find(
        p => p.teamName === auctionState.highestBidder
    )

    if (winner) {
        winner.purse = (winner.purse || 0) - auctionState.currentBid
        winner.playersBought = (winner.playersBought || 0) + 1
        winner.rating = (winner.rating || 0) + auctionState.currentPlayer.rating
    }

    auctionState.currentIndex += 1
    if (auctionState.currentIndex >= auctionPool.length) {
        io.to(roomId).emit('auctionEnded', room)
        delete auctionStateByRoom[roomId]
        return
    }

    const nextPlayer = auctionPool[auctionState.currentIndex]
