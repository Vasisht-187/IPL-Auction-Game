...
export function loadPlayersFromExcel(): AuctionPlayer[] {
  ...
  const players: AuctionPlayer[] = sheet1Data.map((row: any, index: number) => {
    // Try multiple possible name columns and always produce a full name (first + last when available)
    const firstName = (row.FirstName || row['First Name'] || row['first_name'] || row.First || row['First'] || '').toString().trim()
    const lastName = (row.LastName || row['Last Name'] || row['last_name'] || row.Last || row['Last'] || '').toString().trim()
    // Fallbacks if sheet has single "Name" or "FullName" column
    const fallbackFull = (row.Name || row.FullName || row['Full Name'] || row.Player || '').toString().trim()
    let fullName = ''
    if (firstName || lastName) fullName = `${firstName} ${lastName}`.trim()
    else if (fallbackFull) fullName = fallbackFull
    else fullName = ''
